...
const UnconfirmedOrder = ({ index,confirm,data }) => {

  const [cart, setCart] = useState(["Cheeseburger - 2x"])
  ...
  return (
    <div className='p-4 border-2 rounded-lg border-[#93e2ae] bg-white flex flex-col gap-2 '>
        ...
        <button onClick={() => confirm(data.order_id,index)} className='mt-2 px-4 py-2 border-2 rounded-lg border-[#93e2ae] hover:cursor-pointer hover:border-[#355e3b] hover:bg-[#C2F0D1] transition-all'>Accept</button>
      </div>
    </div>
  )
}
