  ...
  return ( 
    <div className='bg-white border-2 border-[#93e2ae] rounded-lg p-4  flex flex-col gap-2 mt-2 m-4'>
        <div className="flex justify-between ">
            ...
            <Link to="/newRestaurant" className="text-2xl rounded-lg border-2 border-[#93e2ae] px-2 pt-4 hover:cursor-pointer hover:bg-[#93e2ae] hover:border-[#355e3b] transition-all">New restaurant</Link>
        </div>
        <div className="border-2 border-[#93e2ae] rounded-lg p-4  flex flex-col gap-2 mt-2">
            {foods.map((x,i) => <ControlRestaurant data={x} />)}            
        </div>
        <div className={`fixed top-0 left-0 w-full h-screen bg-black/20 flex items-center justify-center ${href == "/controlRestaurants" ? "hidden" : ""}`}>
